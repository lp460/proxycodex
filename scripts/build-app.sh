...
if [[ "${UNIVERSAL_BUILD:-no}" == "yes" ]]; then
    ...
    BUILD_DIR="$(swift build -c release --arch arm64 --arch x86_64 \
        --scratch-path "$SCRATCH_DIR" --show-bin-path)"
else
    echo ">> Building release executable…"
    swift build -c release --scratch-path "$SCRATCH_DIR"
    BUILD_DIR="$(swift build -c release --scratch-path "$SCRATCH_DIR" \
        --show-bin-path)"
fi
BIN="$BUILD_DIR/$EXEC"
if [[ ! -x "$BIN" ]]; then
...
SPARKLE_FRAMEWORK="$(find "$REPO_ROOT/$SCRATCH_DIR" -type d -name Sparkle.framework -print -quit)"
